...
for index, banner in enumerate(banners):
    ...
    title = banner.find('div', class_='promotion-card-wrapper').text.strip() if banner.find('div',
                                                                                            class_='promotion-card-wrapper') else None
...
def login():
    ...
    for username, password in zip(usernameArr, passwordArr):
        ...
        password_button.send_keys(Keys.CONTROL + "a")
        ...
        try:
            home_page_element = WebDriverWait(driver, 20).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//span[@class='text-[14px] md:text-[1rem]'][normalize-space()='13,045.00']"))
            )
